...
3. **Gross fees**: attendance pattern × sessions × rates, sibling discounts, recurring extras. One-off extras (registration fee) appear in each child's first attending month and in the annual total, itemised separately so monthly comparison stays fair. As built (task 5): session selection is deterministic and stated — exact-hours session, then hourly rate, then the smallest session covering the day, then a weekly rate, then the longest session with a flagged shortfall; every non-exact choice carries a user-facing assumption. Week bases are 38 (term time) and an assumed 51 (all-year, noted on every line it produces). Refundable extras are shown but excluded from totals.
