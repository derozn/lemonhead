...
| ~~Money-boundary fix (ADR 0008) + UC-guard widening~~ (done, PR #9) | Explanation lines carry raw Pence fields, `apps/web` formats via `pounds()`; type-aware ESLint rule bans Pence arithmetic outside `money.ts`; string tests; UC memory-only enforcement widened beyond `storage.ts`.                                  | Revisit audit findings 1–3    |
| ~~Schemas coverage threshold~~ (done, PR #10)                       | Cover `fee-schedule.ts:229`, add a coverage threshold for `packages/schemas`.                                                                                                                                                                        | Trade-off row, promoted       |
| ~~`next-env.d.ts` tracked-input mutation~~ (done)                   | Fix the first-checkout `next build` cache miss at source.                                                                                                                                                                                            | Trade-off row, promoted       |
| ~~`gross.ts` sketch-first pass~~ (closed: examined, no change)      | One function's before/after, owner verdict before anything wider.                                                                                                                                                                                    | Parked row, promoted          |
| ~~Verbatim citation quotes~~ (done, PR #12)                         | Re-fetch the `quote` fields in all three params files via /verify-rule so each is an exact gov.uk extract.                                                                                                                                           | Revisit audit finding 5       |
| ~~Comparison view (FR4)~~ (done)                                    | Moved forward from Phase 4 by spec amendment v1.2.                                                                                                                                                                                                   | Parked row, promoted          |
...
| Earnings question restructure | The minimum-earnings question still makes parents do the government's arithmetic. Ask rough earnings per parent instead and compare in the engine against the cited thresholds. Schema + engine + form change; revisit after real-user feedback on the plain-English pass |
| Comparison full breakdown     | Owner note at the FR4 verdict: an option to expand the comparison to full line-by-line detail per nursery. Natural home: Phase 4 review UI                                                                                                                                |
