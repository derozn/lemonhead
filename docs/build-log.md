...
## 2026-08-07 — Phase 1.5: the comparison view (FR4)

The last code item of the hardening batch, pulled forward from Phase 4 by the revisit verdict. Storage moved from a single saved nursery to a name-keyed list with an active pointer; a legacy saved nursery migrates silently on first read, and the storage suite now proves the UC money figures stay out of localStorage with multiple nurseries saved, not just one. The page gains a switcher bar (inline two-step delete, no blocking dialogs) and, at two or more saved nurseries, a comparison section: headline cards per nursery over a twelve-row month table with the cheapest cell badged per row. The monthly average divides annual net by twelve inside format.ts, the sanctioned display home; the components themselves order and compare Pence but never do arithmetic on it, which the lint rule now guarantees rather than promises. The fixture pair proves the point of the whole feature: The Orchard is free through October (term-time only) then £122.86 a month against Sunny Bank's steady £72.20, so the cheapest column flips mid-year exactly where a parent would misjudge it. Owner note recorded at the verdict: a full line-by-line comparison option is wanted later; it is on the backlog for the Phase 4 review UI. 272 tests.

The pre-PR review then earned its keep with a blocking find: renaming a nursery to a name another entry already held silently merged the two, and the next delete removed both, real data loss behind a polite confirm UI. The fix refuses name collisions at save time with a plain error instead of overwriting, recorded as ADR 0009 (name-as-identity is a manual-entry-era decision; machine-created records in Phase 3 will need generated ids). The same pass hardened malformed-localStorage reads (parse failures now degrade to empty instead of taking down the page), added the missing page-level wiring test for the comparison section, and gave the two-step delete proper focus management.
