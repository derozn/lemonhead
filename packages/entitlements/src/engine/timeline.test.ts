...
import { perDayHoursDeduction, perHourConditionalFunding } from '@lemonhead/schemas/fixtures';
import { describe, expect, it } from 'vitest';

import { buildTimeline, DEFAULT_HORIZON_MONTHS } from './timeline.ts';

function profileWithDobs(...dobMonths: string[]): FamilyProfile {
  return FamilyProfile.parse({
    children: dobMonths.map((dobMonth) => ({
      dobMonth,
      disabled: false,
...
describe('buildTimeline', () => {
  ...
  it('places an older child in the open-ended oldest band', () => {
    const timeline = buildTimeline(
      profileWithDobs('2022-01'),
      perDayHoursDeduction,
      isoMonth('2026-08'),
    );
    expect(timeline[0]?.children[0]?.ageMonths).toBe(55);
    expect(timeline[0]?.children[0]?.band).toMatchObject({ ageBandId: 'preschool' });
  });
  ...
  it('marks a child not yet born, then in no matching band, then in band', () => {
    ...
    const timeline = buildTimeline(
      profileWithDobs('2026-10'),
      perHourConditionalFunding,
      isoMonth('2026-08'),
    );
    expect(timeline[0]?.children[0]?.band).toEqual({ status: 'not-born' });
    expect(timeline[2]?.children[0]?.band).toEqual({ status: 'no-matching-band' });
    expect(timeline[10]?.children[0]?.band).toEqual({ status: 'no-matching-band' });
    expect(timeline[11]?.children[0]?.band).toMatchObject({ ageBandId: 'under-3' });
  });

  it('supports custom horizons and multiple children', () => {
    const timeline = buildTimeline(
      profileWithDobs('2024-11', '2022-01'),
      perDayHoursDeduction,
      isoMonth('2026-08'),
      3,
    );
